...
sheets <- excel_sheets("./all_Morgue_cleaned.xlsx") 
morgue <- lapply(sheets, FUN = function(x) {
           read_xlsx(path = "./all_Morgue_cleaned.xlsx",
                     sheet = x) })
names(morgue) <- sheets
```

```{r rest, warning = FALSE, message = FALSE}
sheets <- excel_sheets("./all_REST_cleaned.xlsx")  # note the diff location
rest <- lapply(sheets, FUN = function(x) {
           read_xlsx(path = "./all_REST_cleaned.xlsx",
                     sheet = x) })
names(rest) <- sheets
```
