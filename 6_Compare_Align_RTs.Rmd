---
title: "Comparison of Align vs Not Align"
author: "Wesley Burr"
date: "`r Sys.Date()`"
...
```{r}
library("tidyverse")
aligned <- read.csv("pca_ready_all.csv")
unaligned <- read.csv("Unaligned_pca_ready_all.csv")
```

## Compounds Not Previously Detected

```{r}
new <- (aligned$Name)[!(aligned$Name %in% unaligned$Name)]
n_detect1 <- apply(aligned[, -1], MAR = 1, FUN = function(x) { length(which(x > 0)) })
n_detect2 <- apply(unaligned[, -1], MAR = 1, FUN = function(x) { length(which(x > 0)) })
```

Extract the new compounds, check their detect frequencies:

```{r}
new_c <- aligned[aligned$Name %in% new, ]
n_new <- apply(new_c[, -1], MAR = 1, FUN = function(x) { length(which(x > 0)) })
```

So there are 530 'new' compounds detected after the alignment procedure, of
which `r length(which(n_new == 1)` were 1-off detects, and therefore not really
relevant. However, there are `r length(which(n_new > 5))` interesting compounds
that were broadly detected, and had **not** been detected before. 

## Check RTs for Aligned Compounds, Across Detects

This is a bit tricky, because for a compound to be detected, we had to have detected
it in 3x replicates (mostly; sometimes 2x, sometimes 4x), and have it either not
be detected in the Control sample, or be significantly more detected (2.0x S/N)
in the sample, for it to 'survive'. Darshil is interested in how the RTs
and masses show up for each of the detects. This will be somewhat tedious to do,
so we have to be good about our coding habits.

We have `r nrow(aligned)` total compounds to consider. Each one is detected
sometimes. We need a map from Sample back to original sample names, because
each sample is a 1:2, 1:3 or 1:4 map because of coincidences. 
...
##
#
#  join_common_RT: Function which takes output of clean_common above,
#    and merges based on common presence across all replicates of compounds. 
#    In this case, saves the RTs, instead of just the max areas.
#
#  Inputs:
...
##
join_common_RT <- function(compounds) {
  ...
  for(j in 1:n_samp) {
    ...
    subset_compounds[[j]] <- subset_compounds[[j]] %>% select(Name, RT1, RT2, Area, BaseMass, PeakSN)
  }
  ...
  if(n_samp > 1) {
    ...
    common <- subset_compounds[[1]][, c("Name", "RT1", "RT2", "Area", "BaseMass", "PeakSN")]
  }
  names(common) <- c("Name", paste0(c("RT1_", "RT2_", "Area_", "BaseMass_", "PeakSN_"), 
                                    rep(1:n_samp, each = 5)))
  common
}
```
...
save(file = "./Morgue_PCA_wRTs.rda", all_SS)
```
...
save(file = "./REST_PCA_wRT.rda", all_SS)
```
